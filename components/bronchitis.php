...
if (isset($_POST['submit'])) {
    ...
    $query = mysqli_query($conn, "INSERT INTO tbl_bronchitis (symptoms, types, daytime, limitation, nocturnal, rescue, lf, ex, pftgrading, habits, controlled, partially, uncontrolled, updatedBy, centerName, centerCode, patient_id) 
        VALUES ('$symptoms', '$types', '$daytime', '$limitation', '$nocturnal', '$rescue', '$lf', '$ex','$pftgrading','$habits', '$controlled', '$partially', '$uncontrolled', '$updatedBy', '$centerName', '$centerCode', '$patient_id')");
    if ($query) {
...
if (isset($_POST['update'])) {
    ...
    nocturnal = '$nocturnal',rescue = '$rescue',lf = '$lf',ex = '$ex',pftgrading = '$pftgrading',habits = '$habits',controlled = '$controlled',partially = '$partially',uncontrolled = '$uncontrolled' where id = '$sid'");
    if ($query) {
        echo "<script>alert('You have successfully inserted the data');</script>";
